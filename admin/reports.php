<?php
// admin/reports.php (FINAL UI - CONSISTENT LIST STYLE)
...
<div class="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
    ...
    <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
        ...
        <select id="categoryFilter" class="w-full sm:w-48 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none bg-white text-sm">
...
<div id="resultModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden backdrop-blur-sm px-0 sm:px-4">
    <div class="bg-white w-full h-full sm:h-auto sm:rounded-xl shadow-2xl sm:max-w-4xl sm:max-h-[90vh] flex flex-col overflow-hidden transition-all">
        <div id="modal-header" class="p-4 sm:p-6 border-b bg-indigo-50 shrink-0"></div>
        
        <div id="modal-body" class="p-4 sm:p-6 overflow-y-auto bg-gray-50 flex-grow custom-scrollbar"></div>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden backdrop-blur-sm px-4">
    <div class="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm text-center">
        ...
        <div class="flex justify-center gap-3">
            ...
            <button id="confirmDeleteBtn" class="w-full sm:w-auto px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium shadow text-sm">Hapus</button>
        </div>
    </div>
</div>
