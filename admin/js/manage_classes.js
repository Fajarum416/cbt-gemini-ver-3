// admin/js/manage_classes.js (FIXED: DOUBLE SUBMIT REMOVED)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // REVISI: Baris ini DIHAPUS karena di HTML tombol saveBtn sudah ada onclick="saveAllChanges()"
      ...
      const item = (s, type) => `
        ...
        type === "in" ? "border-indigo-200 shadow-sm" : "border-gray-200"
                ...
                <div class="flex items-center gap-3 pointer-events-none select-none">
                    <div class="w-6 h-6 rounded-full flex items-center justify-center ${type === "in" ? "bg-indigo-100 text-indigo-600" : "bg-gray-100 text-gray-400"}">
...
function moveItem(element) {
  ...
  if (parentId === "nonMembersList") {
    ...
    iconUserBg.className = "w-6 h-6 rounded-full flex items-center justify-center bg-indigo-100 text-indigo-600";
    ...
    iconUserBg.className = "w-6 h-6 rounded-full flex items-center justify-center bg-gray-100 text-gray-400";
...
function saveAllChanges() {
  const mIds = Array.from(document.getElementById("membersList").querySelectorAll(".s-item")).map((i) => i.dataset.id);
  const pl = {
  ...
  // Pengecekan sederhana untuk mencegah double click cepat
...
function fetchClasses() {
  ...
  container.innerHTML = '<div class="col-span-full text-center p-8 text-gray-500"><i class="fas fa-spinner fa-spin mr-2"></i> Memuat data...</div>';
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col overflow-hidden">
                    <div class="p-5 border-b border-gray-50 flex justify-between items-start bg-gradient-to-br from-white to-gray-50">
                        <div class="overflow-hidden pr-2">
                            <h3 class="font-bold text-gray-800 text-lg truncate" title="${c.class_name}">${c.class_name}</h3>
                            <p class="text-xs text-gray-500 mt-1 truncate">${c.description || "-"}</p>
                        </div>
                        <div class="shrink-0">
