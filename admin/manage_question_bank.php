...
// Cek apakah ini permintaan data (fetch) atau aksi (submit/delete)
if (isset($_GET['fetch_list']) || isset($_POST['action'])) {
    
    // Hanya butuh koneksi DB, bukan seluruh header HTML
    ...
    if (isset($_GET['fetch_list'])) {
        ...
        if (!empty($category)) { $sql .= " AND category = ?"; $types .= 's'; $params[] = $category; }
        if (!empty($search)) { $sql .= " AND question_text LIKE ?"; $types .= 's'; $params[] = '%' . $search . '%'; }
    ...
    if (isset($_POST['action'])) {
        ...
        function handle_upload($file_key, $upload_dir) {
            if (isset($_FILES[$file_key]) && $_FILES[$file_key]['error'] == 0) {
                ...
                $target_path = '../uploads/' . $upload_dir . '/' . $filename;
        ...
        $options_assoc = [];
        $char_code = 65;
        foreach($options_text as $opt) { $options_assoc[chr($char_code++)] = $opt; }
        $options_json = json_encode($options_assoc);
        
        $image_path = handle_upload('image_file', 'images');
        $audio_path = handle_upload('audio_file', 'audio');
        ...
        if ($action == 'add_question') {
            ...
            if ($audio_path) { $sql .= ", audio_path=?"; $params[] = $audio_path; $types .= "s"; }
            $sql .= " WHERE id=?";
            $params[] = $question_id;
            $types .= "i";
...
// --- BAGIAN INI HANYA UNTUK TAMPILAN HALAMAN NORMAL ---
$page_title = 'Manajemen Bank Soal';
require_once 'header.php'; // Header dimuat setelah blok AJAX

// **REVISI DITERAPKAN DI SINI**
// Kode ini secara dinamis mengambil semua kategori yang unik dari database.
// Hasilnya kemudian digunakan untuk mengisi pilihan pada dropdown filter di bawah.
// Ini memastikan filter selalu menampilkan kategori yang relevan.
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div id="options-container" class="space-y-3">
                        <!-- Opsi dinamis diisi oleh JavaScript -->
                    </div>
...
// --- Fetch & Render Questions Table (AJAX) ---
let currentPage = 1;
